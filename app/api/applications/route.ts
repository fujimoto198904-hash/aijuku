  ...
  try {
    ...
    const application = await createMemberApplication({
      ...
      offerSnapshot: {
        ...
        price: plan.price,
        area: plan.area,
        enrollmentFee: `${sharedFees.entrance}（${sharedFees.entranceCampaign}・${sharedFees.entranceRegular}）`,
        pricingNote: `${sharedFees.entranceCondition} 税込区分・支払方法・変更取消条件は確定前に提示`,
      },
    });
