...
const sections = [
  [
    '1. 取得する情報',
    'ユーザー名、表示名、規約への同意日時、パスワード・復旧コードの検証用データ、ログイン記録、学習記録、投稿・返信とその操作記録を取得します。メールなしでも登録できます。メール登録では確認済みメールを、Google登録ではGoogleのユーザー識別子と確認済みメールを利用します。Googleカレンダーや連絡先へのアクセスは求めません。',
  ],
  [
    '2. 利用目的',
  ...
  [
    'プロフィール・フォロー・いいね・DM',
    '公開を選んだプロフィールの名前・自己紹介・今後の投稿との紐付け・公開中のフォロー関係を表示します。過去の別名の投稿を自動でまとめません。いいねは操作した会員と投稿を保存し、件数を公開します。DMの本文は当事者だけの画面に表示し、一般公開しません。通報した場合、そのメッセージ1通と理由を運営が確認します。ブロックしても過去の会話の確認と通報ができます。プロフィールを非公開にしても、すでに公開した投稿や送信済みのDMは自動で削除されません。',
  ],
] as const;
