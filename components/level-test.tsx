...
const questions = [
  ...
  {
    question:
      "藤本実学塾の「AIだけを使って課題を解く」で、人が担う役割として最も適切なのは？",
    choices: [
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_420px]">
        ...
        >
          ...
          <p className="mt-5 max-w-xl text-sm leading-7 text-quiet">
            {passed
              ? "情報確認と安全性の基礎的な判断ができています。これは体験結果であり、正式な科目の修了や認定にはなりません。"
              : "確認の目安は8問正解です。間違えた問題の解説を読み、もう一度試してみましょう。AIを使う力には、出力を疑い、止める判断も含まれます。"}
          </p>
        ...
        <aside className="rounded-[30px] bg-ink p-6 text-ivory shadow-[0_24px_80px_rgba(8,16,25,0.18)] sm:p-8">
          ...
          <p className="mt-2 text-base font-bold tracking-[0.05em]">
            藤本実学塾
          </p>
          <div className="my-12 text-center">
            <ShieldCheck
              ...
              aria-hidden="true"
            />
            <p className="mt-5 text-xs font-bold tracking-[0.08em] text-white/80">
              基礎の確認結果
            </p>
            <p className="mt-3 font-mono text-5xl font-black text-future-mint">
              BASIC
            </p>
            <p className="mt-3 text-sm font-bold">AI安全活用・基礎理解</p>
          </div>
          <div className="border-t border-white/10 pt-5 text-xs leading-6 text-white/75">
            <p>DEMO ID</p>
            <p className="mt-1 font-mono text-white/85">
              FJJ-DEMO-{String(score).padStart(2, "0")}
            </p>
            <p className="mt-5">
              この画面は体験用で、正式な修了記録・証明書として発行、保存されません。
            </p>
          </div>
        </aside>
      </div>
