...
- 9月7日追加（未公開）：フィード内でコメントを読み書き・削除。本文は140文字から同じ場所で展開、新規投稿はURL込み1,000文字まで。既存の長文や共有用URLは維持。[仕様と確認範囲](docs/INLINE_COMMENTS_2026_09_07.md)。
...
- 教科書の質問は「質問を書く」から一つの投稿画面へ（未公開）。課題名を表示し、ログイン・登録・規約同意後も同じ質問画面へ戻ります。
- 投稿画面は本文＋任意の写真・URLへ簡略化（未公開）。タイトル入力と毎回の公開確認チェックを外し、投稿ボタンで公開。画像は投稿時だけ送信し、500KB以下／1枚に縮小、保存量と回数を制限します。0020はローカルのみ適用。[仕様と検証](docs/POSTING_2026_09_06.md)。
