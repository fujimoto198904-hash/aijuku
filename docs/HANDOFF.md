...
会員ルートは未ログイン時のリダイレクトに加え、`/signin-with-chatgpt`からローカルの模擬ログインを行い、初回登録、課題の「あとでやる」保存、完了・未完了への切替、本人別申込、管理者拒否まで確認します。その後、`npm run verify`が成功することを確認します。
...
全730課題の開始素材として、美容室、建設業、不動産会社の架空データパックを生成しました。短いTXTを使う106課題では、教材本文を開くと選択中の架空会社の中身を最初から表示し、「このTXTをコピー」でChatGPTへそのまま貼れます。TXTだけの課題ではZIP取得を出さず、PDF・Word・Excel・画像等を併用する課題だけ、TXTの下でZIP取得と添付方法を案内します。入力方法や作業画面は採点しません。Chatの成果物は本人がコピーまたはダウンロードして保存し、WorkはAIへ保存を頼んだ後に本人が開きます。ZIPは使う端末の「展開」「解凍」で開きます。Windowsの右クリックやMacのダブルクリックは補足例であり、特定OSの操作は完成条件にしません。開始素材と大量の会社データはありますが、各課題を実際にChatGPTで行って作った完成見本画像、`.xlsx`、`.pptx`、サイト、アプリの一式は未整備です。
...
2. 管理画面を公開する前に、Sitesの実行環境へ`ADMIN_EMAILS`と`INSTRUCTOR_EMAILS`を設定します。実在メールアドレスはリポジトリへ保存しません。
...
4. Sites本番で、ログアウト、無料登録、既存会員の再同意、マイページ、課題の保存・完了・未完了、申込、管理ページ、本人記録、講師確認、URL共有プロフィール、規約類を確認します。
5. Sitesの会員E2Eが成功した後だけ、同じcommitを`origin/main`へpushします。これでVercelの無料会員訴求と会員導線が公開されます。
6. Vercel子と正式URLで、トップ、Web教科書、会員ルートのSites転送、静的素材を確認します。
...
- D1にはSign in with ChatGPTのサイト内ユーザーID、確認済みメール、表示名、同意記録、課題のブックマーク・本人完了状態、申込内容、成果物説明・外部URL・権利確認日時・公開設定、講師確認を保存します。APIはクライアントから会員IDを受け取らず、認証済み本人IDで絞り込みます。共有プロフィールにメールとChatGPT利用者IDは表示しません。
