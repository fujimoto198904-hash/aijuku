...
- 運営用とデモ用の初期アカウントは作成済み。ログインID、パスワード、過去の初期化用トークンの実値はコードや本資料に記録しない。初期アカウント作成APIは恒久停止しており、復活させない。
...
2. Sitesの実行環境に必要な`AUTH_PASSWORD_PEPPER`、`ADMIN_EMAILS`、`INSTRUCTOR_EMAILS`、必要時の`AUTH_OWNER_LOGIN_ID`がそのまま維持されることを確認します。実在メールアドレス、パスワード、pepperの実値はリポジトリや作業ログへ保存しません。
