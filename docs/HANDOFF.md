...
| 未実施                   | 運営者本人の新パスワード確定、別の一般会員による全行程、受講者端末でのChatGPT実践QA           |
...
2026年9月3日に、ID・パスワード認証、初回変更、オンボーディング、閲覧専用デモを既存Sitesへ追加公開しました。本番D1の`0008`適用と4つの認証・オンボーディングテーブル、運営者のChatGPT本人確認から強制変更画面まで、既存の管理権限、デモログインと9系統の書込み拒否、外部からの本人確認ヘッダー偽装拒否を確認しました。初期アカウント作成後は`AUTH_BOOTSTRAP_TOKEN`をSites環境から削除しています。
...
- 新規会員の初回パスワードには、オンボーディングで入力した生年月日8桁を使う。初回パスワードで入る時は、同じメールアドレスのSign in with ChatGPT本人確認も必須にする。発行から72時間で失効し、登録直後または初回ログイン時に本人専用のパスワードへ変更するまで、通常の会員機能は利用できない。
...
- 運営用とデモ用の初期アカウントは、秘密環境変数で保護した一時的な初期化APIから作成する。ログインID、パスワード、初期化用トークンの実値はコードや本資料に記録しない。作成後は初期化用トークンをSites環境から削除し、同じコード版を再公開する。
...
| `app/api/auth/`                                    | ID・パスワードのログイン、初回変更、ログアウト、一時的な初期化API                                    |
...
- 本番URLでの登録、同意、ブックマーク、完了・未完了切替、申込、本人別表示、成果物提出、講師確認、URL共有、共有停止、管理者表示のE2E確認
...
`0008`は、ID・パスワード認証アカウント、ハッシュ化したセッション、ログイン回数制限用の保護済み識別子と試行回数、初回オンボーディングの4項目を追加します。生年月日、初回入力の8桁、パスワード原文、セッショントークン原文、IPアドレスそのものは保存しません。追加認証版はまだSites本番へ未公開で、`0008`も本番適用待ちです。公開前に空のローカルD1で適用と認証検査を確認し、公開後にSites本番のテーブル、マイグレーション履歴、一般会員・運営・デモの通しQAを実測して本資料を更新します。
...
2. Sitesの実行環境へ、パスワードハッシュ用の`AUTH_PASSWORD_PEPPER`、初期アカウント作成中だけ使う`AUTH_BOOTSTRAP_TOKEN`、管理権限用の`ADMIN_EMAILS`と`INSTRUCTOR_EMAILS`を設定します。必要な場合だけ`AUTH_OWNER_LOGIN_ID`も設定します。実在メールアドレス、パスワード、トークン、pepperの実値はリポジトリや作業ログへ保存しません。
...
4. 一時的な初期化APIで運営用と閲覧専用デモを作成します。作成結果に原文パスワードやハッシュを含めず、同じリクエストで二重作成しないことを確認します。完了後は`AUTH_BOOTSTRAP_TOKEN`をSites環境から削除し、同じコード版を再公開して初期化APIが利用不可になったことを確認します。
...
- `AUTH_PASSWORD_PEPPER`はSitesの秘密環境変数にだけ設定します。`AUTH_BOOTSTRAP_TOKEN`は初期化中だけ設定し、完了後に削除して再公開します。2026年9月3日の初期アカウント作成後に削除済みです。
- パスワードを忘れた場合のメール相談は登録メールアドレスから受け付けますが、自動再設定メールと具体的な復旧方法は準備中です。
- `ADMIN_EMAILS`、`INSTRUCTOR_EMAILS`、必要時の`AUTH_OWNER_LOGIN_ID`はSitesの実行環境にだけ設定し、リポジトリへ実在メールアドレスを保存しません。管理者・講師が未設定の時は管理画面を拒否し、講師には申込者メールを返しません。
...
- [x] 運営用アカウントのChatGPT本人確認、初回強制変更画面、管理権限をSites本番で確認（新パスワード確定は本人へ引継ぎ）
