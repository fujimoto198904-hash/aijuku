...
| 正式ブランチ             | `main`                                                                            |
| 今回の追加作業           | ID・パスワード認証、初回変更、会員オンボーディング、閲覧専用デモをソースへ追加。本番検証待ち |
| 教材実数                 | 全730本文、73/73章、`lessonDrafts: 730 / outlines: 0`                             |
| 検証                     | `npm run verify`、Sites本番、Vercel READY、正式URL、185配信素材の確認に成功       |
| 未実施                   | 本番D1移行と新認証のSites公開、一般会員・運営・デモの通しQA、受講者端末でのChatGPT実践QA       |
...
上記は追加する認証版の正本設計です。「本番利用可」とするのは、ローカルのマイグレーション・認証検査・本番ビルド、Sitesの本番D1移行と公開、実際の一般会員・運営・デモの通しQAまで成功した後です。
...
2. Sitesの実行環境へ、パスワードハッシュ用の`AUTH_PASSWORD_PEPPER`、初期アカウント作成中だけ使う`AUTH_BOOTSTRAP_TOKEN`、管理権限用の`ADMIN_EMAILS`と`INSTRUCTOR_EMAILS`を設定します。実在メールアドレス、パスワード、トークン、pepperの実値はリポジトリや作業ログへ保存しません。
...
- `AUTH_PASSWORD_PEPPER`と`AUTH_BOOTSTRAP_TOKEN`はSitesの秘密環境変数にだけ設定します。初期化完了後は`AUTH_BOOTSTRAP_TOKEN`を削除して再公開します。
...
- [x] 公開URL確認
- [ ] 新認証用D1マイグレーションを空のローカルD1へ適用し、テーブル・制約・インデックスを確認
- [ ] 新規ChatGPT本人確認 → オンボーディング → 初回ログイン → パスワード変更 → マイページをSites本番で通しQA
- [ ] 運営用アカウントの初回強制変更と管理権限をSites本番で確認
- [ ] デモアカウントの申込・進捗・プロフィール・管理書込みが全APIで拒否されることを確認
- [ ] 本番の初期アカウント作成後に`AUTH_BOOTSTRAP_TOKEN`をSites環境から削除し、再公開後の拒否を確認
